...
class PhocaDownloadCpViewPhocaDownloadCats extends JView
{
	...
	protected $state;
	protected $tmpl;
	//protected $_context 	= 'com_phocadownload.phocadownloadc';
	...
	protected function addToolbar() {
		...
		$canDo	= PhocaDownloadCatsHelper::getActions($state->get('filter.category_id'));
		
		JToolBarHelper::title( JText::_( 'COM_PHOCADOWNLOAD_CATEGORIES' ), 'category.png' );
		if ($canDo->get('core.create')) {
			JToolBarHelper::addNew('phocadownloadcat.add','JTOOLBAR_NEW');
		}
		...
		if ($canDo->get('core.edit.state')) {
			...
			JToolBarHelper::custom('phocadownloadcats.publish', 'publish.png', 'publish_f2.png','JTOOLBAR_PUBLISH', true);
			JToolBarHelper::custom('phocadownloadcs.unpublish', 'unpublish.png', 'unpublish_f2.png', 'JTOOLBAR_UNPUBLISH', true);
			//JToolBarHelper::custom( 'phocadownloadcs.approve', 'approve.png', '', 'COM_PHOCADOWNLOAD_APPROVE' , true);
			//JToolBarHelper::custom( 'phocadownloadcs.disapprove', 'disapprove.png', '',  'COM_PHOCADOWNLOAD_NOT_APPROVE' , true);
		}

		if ($canDo->get('core.delete')) {
			JToolBarHelper::deleteList( JText::_( 'COM_PHOCADOWNLOAD_WARNING_DELETE_ITEMS' ), 'phocadownloadcats.delete', 'COM_PHOCADOWNLOAD_DELETE');
		}
		JToolBarHelper::divider();
